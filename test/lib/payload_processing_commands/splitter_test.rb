...
class PayloadProcessingCommands::SplitterTest < MiniTest::Unit::TestCase

  def test_it_finds_all_words_in_sentence
    s = PayloadProcessingCommands::Splitter.new(type: :word)
    payload = 'Some, funny, : []( "sentence"'
    assert_equal(["Some", "funny", "sentence"], s.perform(payload))
  end

  def test_it_finds_sentences
    s = PayloadProcessingCommands::Splitter.new(type: :sentence)
  ...
  def test_it_find_urls
    s = PayloadProcessingCommands::Splitter.new(type: :url)
  ...
  def test_it_uses_custom_rules
    s = PayloadProcessingCommands::Splitter.new(type: :custom, rule: /(\d+)/)
    payload = "111aaa2222"
  ...
  def test_it_uses_word_processing_utility
    s = PayloadProcessingCommands::Splitter.new(type: :word, word_utility: true)
